...
def main(args):
    ...
    for mode in args.modes:
        ...
        for fname in data_in.get(mode):
            key = "{}/{}".format(mode, fname)
            # print(f"\n[info] processing {key}\n")
            x = data_in.get(key)
            x = np.array(x)
    ...
    for key, y in distributed_result:
...
def get_args():
    ...
    parser.add_argument("--data_in", type=str, default=os.path.join(main_data_path, "keypoints-01-raw_wb.h5"))
